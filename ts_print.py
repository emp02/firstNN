...
class writeCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        keys = list(logs.keys())
        summary = f"\nEpoch {epoch+1}/{TRAIN_EPOCHS}:\n"
        for key in keys:
            summary += f"{key}: {logs[key]:.3f} - "
        print(summary[:-3], file=open('PRINT.txt', 'a'))
...
class Net():
    def __init__(self, input_shape):
        ...
        self.model.add(layers.Conv2D(54, 3, activation = 'relu'))
        # In our example, we are now at 10 x 10 x 16.
        self.model.add(layers.MaxPooling2D(pool_size = 2))
        self.model.add(layers.Conv2D(486, 3, activation = 'relu'))
...
results = net.model.fit(trainX, trainY, validation_data=(testX, testY), shuffle = True, epochs = TRAIN_EPOCHS, batch_size = BATCH_SIZE_TRAIN, validation_batch_size = BATCH_SIZE_TEST, verbose = 1, callbacks=[writeCallback()])
#net.model.save('model1')
